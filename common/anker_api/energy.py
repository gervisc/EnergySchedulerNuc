...
def energy_daily(  # noqa: C901
    ...
    if SolixDeviceType.SOLARBANK.value in devTypes:
        ...
        if fromFile:
            resp = (
                self.apisession.loadFromFile(
                    Path(self.testDir())
                    / f"{API_FILEPREFIXES['energy_solarbank']}_{siteId}.json"
                )
            ).get("data", {})
        else:
            resp = self.energy_analysis(
                siteId=siteId,
                deviceSn=deviceSn,
                rangeType="week",
                startDay=startDay,
...
def device_pv_energy_daily(
    ...
    if fromFile:
        ...
        resp = self.get_device_pv_statistics(
            deviceSn=deviceSn,
            rangeType="week",
            startDay=startDay,
            endDay=startDay + timedelta(days=numDays - 1),
...
def get_device_charge_order_stats(
    self: AnkerSolixApi,
    deviceSn: str,
    rangeType: str | None = None,
    startDay: datetime | None = None,
