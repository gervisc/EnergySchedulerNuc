...
class AnkerSolixApi(AnkerSolixBaseApi):
    ...
    def __init__(
        ...
        self.authenticate = self.apisession.authenticate
        ...
        if sn := devData.pop("device_sn", None):
            ...
            if calc_capacity:
                ...
                if device.get("data_valid", True):
                    ...
                    # get total SOC from API cache
                    soc = device.get("battery_soc", "")
    ...
    def update_sites(
        ...
        resp = poll_sites(self, siteId=siteId, fromFile=fromFile, exclude=exclude)
    ...
    def update_site_details(
        ...
        return poll_site_details(self, fromFile=fromFile, exclude=exclude)

    def update_device_energy(
        self, fromFile: bool = False, exclude: set | None = None
    ) -> dict:
        """Add/Update energy details in api sites cache structure."""
        return poll_device_energy(self, fromFile=fromFile, exclude=exclude)

    def update_device_details(
        self, fromFile: bool = False, exclude: set | None = None
    ) -> dict:
        """Create/Update device details in api devices cache structure."""
        resp = poll_device_details(self, fromFile=fromFile, exclude=exclude)
    ...
    def get_homepage(self, fromFile: bool = False) -> dict:
        ...
        """
        if fromFile:
            resp = self.apisession.loadFromFile(
                Path(self.testDir()) / f"{API_FILEPREFIXES['homepage']}.json"
            )
        else:
            resp = self.apisession.request("post", API_ENDPOINTS["homepage"])
        return resp.get("data") or {}

    def get_user_devices(self, fromFile: bool = False) -> dict:
        ...
        if fromFile:
            ...
            resp = self.apisession.request("post", API_ENDPOINTS["user_devices"])
        return resp.get("data") or {}

    def get_charging_devices(self, fromFile: bool = False) -> dict:
    ...
    def get_solar_info(self, solarbankSn: str, fromFile: bool = False) -> dict:
    ...
    def set_power_cutoff(
        ...
        if toFile:
            ...
            if filedata and not self.apisession.saveToFile(
                Path(self.testDir())
                / f"{API_FILEPREFIXES['get_cutoff']}_modified_{deviceSn}.json",
                data={
                    "code": 0,
        ...
        return self.get_power_cutoff(deviceSn=deviceSn, fromFile=toFile)

    def set_station_parm(
        self,
        siteId: str | None = None,
        deviceSn: str | None = None,
    ...
    def set_device_attributes(
        ...
        if toFile:
            ...
            if not (
                data := self.get_device_attributes(
            ...
            if not self.apisession.saveToFile(
                Path(self.testDir())
                / f"{API_FILEPREFIXES['get_device_attributes']}_modified_{deviceSn}.json",
                data={
                    "code": 0,
    ...
    def get_power_limit(self, siteId: str, fromFile: bool = False) -> dict:
    ...
    def set_power_limit(
        ...
        attr_resp = None
        if not isinstance(
            attr_resp := self.set_device_attributes(
                deviceSn=deviceSn,
                attributes=data,
        ...
        if toFile and (
            resp := self.get_power_limit(siteId=siteId, fromFile=toFile)
        ):
            for device in [
                d
        ...
        return {"device_attributes": attr_resp} | self.get_power_limit(
    ...
    def get_ai_ems_runtime(self, siteId: str, fromFile: bool = False) -> dict:
    ...
    def get_device_pv_status(
        ...
        if fromFile:
            ...
            resp = {}
            for sn in sns.split(","):
                sn_resp = self.apisession.loadFromFile(
                    Path(self.testDir())
    ...
    def get_device_pv_total_statistics(
    ...
    def get_device_pv_price(self, deviceSn: str, fromFile: bool = False) -> dict:
    ...
    def set_device_pv_price(
        ...
        if not (details := (self.sites.get(siteId) or {}).get("site_details") or {}):
            data = self.get_device_pv_price(deviceSn=deviceSn, fromFile=toFile)
            if tiers := data.get("tieredElecPrices") or []:
                details = {
                    "price": (tiers[0] or {}).get("price"),
                    "site_price_unit": data.get("currencyUnit") or "",
        ...
        if toFile:
            ...
            if not isinstance(code, int) or int(code) != 0:
                return False
        # update the data in api dict and return active data
        return self.get_device_pv_price(deviceSn=deviceSn, fromFile=toFile)

    def set_device_pv_power(
        self,
        deviceSn: str,
        limit: int,
        toFile: bool = False,
        ...
        if toFile:
            ...
            if not (
                data := self.get_device_pv_total_statistics(
            ...
            if not isinstance(code, int) or int(code) != 0:
                return False
        # update the data in api dict and return active data
        return self.get_device_pv_total_statistics(
            deviceSn=deviceSn, fromFile=toFile
        )
