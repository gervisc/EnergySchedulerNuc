...
from .helpers import RequestCounter, generateTimestamp, md5
...
class AnkerSolixClientSession:
    ...
    def __init__(
        ...
        self._session: ClientSession = websession or ClientSession()
    ...
    def authenticate(self, restart: bool = False) -> bool:
        ...
        if Path(self._authFile).is_file():
            data = self.loadFromFile(self._authFile)
            ...
            with open(self._authFile, "w", encoding="utf-8") as authfile:
                authfile.write(json.dumps(data, indent=2, skipkeys=True))
    ...
    def request(  # noqa: C901
        ...
        *,
        ...
        if (
            ...
            self.authenticate(restart=True)
        ...
        self._wait_delay(endpoint=endpoint)
        ...
        # Exception from ClientSession based on standard response status codes
        except (ClientError, RequestTimeout) as err:
            ...
            if resp.status in [401, 403]:
                ...
                if not self._retry_attempt:
                    ...
                    if self.authenticate(restart=True):
                        return self.request(
    ...
    def loadFromFile(self, filename: str | Path) -> dict:
    ...
    def saveToFile(self, filename: str | Path, data: dict | None = None) -> bool:
    ...
    def deleteModifiedFile(self, filename: str | Path) -> bool:
