...
from requests.exceptions import RequestException as ClientError
...
class AnkerSolixBaseApi:
    ...
    def update_sites(
        ...
        if siteId and (self.sites.get(siteId) or {}):
            ...
            sites = self.get_site_list(fromFile=fromFile)
        ...
        for site in sites.get("site_list", []):
            if myid := site.get("site_id"):
                ...
                scene = self.get_scene_info(myid, fromFile=fromFile)
                mysite.update(scene)
    ...
    def get_site_rules(self, fromFile: bool = False) -> dict:
        ...
        if fromFile:
            ...
            resp = self.apisession.request("post", API_ENDPOINTS["site_rules"])
        return resp.get("data") or {}

    def get_site_list(self, fromFile: bool = False) -> dict:
        ...
        if fromFile:
            ...
            resp = self.apisession.request("post", API_ENDPOINTS["site_list"])
        return resp.get("data") or {}

    def get_site_details(self, siteId: str, fromFile: bool = False) -> dict:
    ...
    def get_scene_info(self, siteId: str, fromFile: bool = False) -> dict:
    ...
    def get_bind_devices(self, fromFile: bool = False) -> dict:
        ...
        if fromFile:
            ...
            resp = self.apisession.request("post", API_ENDPOINTS["bind_devices"])
        ...
        for device in data.get("data") or []:
            ...
            if not device.get("device_name") and "products" not in self.account:
                self._update_account(
                    {"products": self.get_products(fromFile=fromFile)}
                )
    ...
    def get_auto_upgrade(self, fromFile: bool = False) -> dict:
    ...
    def set_auto_upgrade(self, devices: dict[str, bool]) -> bool | dict:
        ...
        # get actual settings
        settings = self.get_auto_upgrade()
        ...
        if change_list:
            ...
            resp = self.get_auto_upgrade()
        return resp

    def get_wifi_list(self, siteId: str, fromFile: bool = False) -> dict:
        """Get the wifi list.
    ...
    def get_message_unread(self, fromFile: bool = False) -> dict:
    ...
    def get_product_platforms_list(self, fromFile: bool = False) -> list:
    ...
    def get_third_platforms_list(self, fromFile: bool = False) -> list:
    ...
    def get_hes_platforms_list(self, fromFile: bool = False) -> list:
    ...
    def get_products(self, fromFile: bool = False) -> dict:
        ...
        try:
            for platform in self.get_product_platforms_list(fromFile=fromFile):
            ...
            for platform in self.get_hes_platforms_list(fromFile=fromFile):
            ...
            #     self.apisession.nickname,
            # )
            # for platform in self.get_third_platforms_list(fromFile=fromFile):
            #     plat_name = platform.get("name") or ""
    ...
    def get_co2_ranking(self, siteId: str, fromFile: bool = False) -> dict:
    ...
    def get_price_providers(self, model: str, fromFile: bool = False) -> dict:
    ...
    def get_currency_list(self, fromFile: bool = False) -> dict:
    ...
    def set_site_price(
        ...
        if not (details := (self.sites.get(siteId) or {}).get("site_details") or {}):
            details = self.get_site_price(siteId=siteId, fromFile=toFile)
        if not details or not isinstance(details, dict):
            return False
        # Validate parameters
        provider = (
        ...
        return self.get_site_price(siteId=siteId, fromFile=toFile)
