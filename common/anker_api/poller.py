...
def poll_sites(  # noqa: C901
    ...
    if siteId and (api.sites.get(siteId) or {}):
        ...
        sites: dict = {
            "site_list": [
                ...
                for s in (
                    (
                        (api.get_site_list(fromFile=fromFile)).get("site_list")
                        or []
                    )
                    + [
    ...
    for site in sites.get("site_list", []):
        if myid := site.get("site_id"):
            ...
            if "currency_list" not in api.account and (
                ...
                data = api.get_currency_list(fromFile=fromFile)
                api._update_account(
                    {
            ...
            if (
                ...
                api._update_account(
                    {"products": api.get_products(fromFile=fromFile)}
                )
            # Routines for virtual site types
            if (site_Type := mysite.get("site_type")) == SolixDeviceType.VIRTUAL.value:
            ...
            # Update scene info for other site types and extract values for device updates
                ...
                scene = api.get_scene_info(myid, fromFile=fromFile)
    ...
    if inverters := [
        ...
        api.get_device_pv_status(devices=inverters, fromFile=fromFile)
...
def poll_site_details(
    ...
    if {ApiCategories.account_info} - exclude:
        ...
        api.get_message_unread(fromFile=fromFile)
    for site_id, site in api.sites.items():
        ...
        if site.get("site_type") == SolixDeviceType.VIRTUAL.value:
            ...
            if (api.devices.get(deviceSn) or {}).get(
                ...
                api.get_device_pv_total_statistics(
                ...
                if {ApiCategories.site_price} - exclude:
                    ...
                    api.get_device_pv_price(deviceSn=deviceSn, fromFile=fromFile)
        ...
        if site.get("site_type") in [SolixDeviceType.SOLARBANK.value]:
            ...
            if site.get("site_admin", False):
                ...
                if {ApiCategories.site_price} - exclude:
                    ...
                    api.get_site_price(siteId=site_id, fromFile=fromFile)
                ...
                if {SolixDeviceType.SOLARBANK.value} - exclude:
                    ...
                    api.get_power_limit(siteId=site_id, fromFile=fromFile)
            ...
            if not ({ApiCategories.solarbank_energy} & exclude):
                ...
                api.get_co2_ranking(siteId=site_id, fromFile=fromFile)
            ...
            if site.get("power_site_type") in [12]:
                ...
                api.get_ai_ems_runtime(siteId=site_id, fromFile=fromFile)
            ...
            if {ApiCategories.site_price} - exclude:
                for model in {
                    ...
                    if (datetime.now().strftime("%Y-%m-%d")) != (
                        ...
                        api.get_price_providers(model=model, fromFile=fromFile)
...
def poll_device_details(  # noqa: C901
    api: AnkerSolixApi, fromFile: bool = False, exclude: set | None = None
) -> dict:
    ...
    api.get_bind_devices(fromFile=fromFile)
    ...
    for sn, device in api.devices.items():
        ...
        if dev_Type == SolixDeviceType.INVERTER.value and not site_id:
            ...
            if {SolixDeviceType.INVERTER.value} - exclude:
                api.get_device_pv_status(devices=sn, fromFile=fromFile)
        # Fetch details that only work for site admins and real sites
            ...
            if site_id not in site_wifi:
                ...
                site_wifi[site_id] = (
                    api.get_wifi_list(siteId=site_id, fromFile=fromFile)
            ...
            if dev_Type in ({SolixDeviceType.SOLARBANK.value} - exclude):
                ...
                if (device.get("generation") or 0) < 2:
                    ...
                    # api.get_ota_update(deviceSn=sn, fromFile=fromFile)
                    ...
                    if {ApiCategories.solarbank_solar_info} - exclude:
                        ...
                        api.get_solar_info(solarbankSn=sn, fromFile=fromFile)
...
def poll_device_energy(  # noqa: C901
    api: AnkerSolixApi, fromFile: bool = False, exclude: set | None = None
) -> dict:
    ...
    for site_id, site in api.sites.items():
        if site_id.startswith(SolixDeviceType.VIRTUAL.value):
            ...
            if (
                ...
                api.refresh_pv_forecast(siteId=site_id, fromFile=fromFile)
